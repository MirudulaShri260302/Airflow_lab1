...
from airflow import configuration as conff
```
The script imports essential components for defining and running an Airflow workflow. DAG, PythonOperator, and BranchPythonOperator are used to structure and control task execution. datetime and timedelta handle scheduling and timing operations. Custom functions from src.lab—load_data, data_preprocessing, build_save_model, and load_model_elbow—manage data handling, preprocessing, model training, and selection. The airflow.configuration module (conff) allows access to Airflow’s environment settings.
...
#### Task to build the "Branching logic function
``` python
...
```
he branch_decision function uses model accuracy to determine workflow direction—if accuracy ≥ 0.8, it proceeds to load_model_task (which depends on build_save_model_task and runs load_model_elbow), otherwise it triggers model retraining.

#### Task to create a branch "branch_decision"
``` python
load_model_task = PythonOperator(
    task_id='load_model_task',
    python_callable=load_model_elbow,
...
```
Defines an Airflow PythonOperator task load_model_task that runs load_model_elbow using the saved model path "model.sav" and the upstream build_save_model_task’s output as inputs.
...
```
Sets a linear pipeline load_data_task → data_preprocessing_task → build_save_model_task → branch_task, after which branch_task conditionally fans out to either load_model_task or retrain_model_task based on the branch decision.
...
>>>>>>> 5a6bd5c (Initial commit: Added Airflow Lab 1 DAG and setup files)
